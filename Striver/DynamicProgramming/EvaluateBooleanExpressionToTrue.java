...
public class EvaluateBooleanExpressionToTrue {
    ...
    static int evaluateExpWays(String exp) {
        ...
        return (int) helperMem(exp, 0, n - 1, 1, dp);
    }

    static long helperMem(String exp, int i, int j, int isTrue, Long[][][] dp) {
        ...
        for (int ind = i + 1; ind <= j - 1; ind += 2) {
            long lT = helperMem(exp, i, ind - 1, 1, dp);
            long lF = helperMem(exp, i, ind - 1, 0, dp);
            long rT = helperMem(exp, ind + 1, j, 1, dp);
            long rF = helperMem(exp, ind + 1, j, 0, dp);
    ...
    static int helperTab(String exp) {
        int n = exp.length();
